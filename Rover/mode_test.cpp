...
bool ModeTest::_enter()
{
    ...
    if (!g2.wp_nav.set_desired_location_to_stopping_location()) {
        return false;
        gcs().send_text(MAV_SEVERITY_INFO,"HELLO");
...
float ModeTest::get_distance_to_destination() const
{
    switch (_guided_mode) {
    case Guided_WP:
...
bool ModeTest::reached_destination() const
{
    switch (_guided_mode) {
    case Guided_WP:
...
bool ModeTest::set_desired_speed(float speed)
{
    switch (_guided_mode) {
    case Guided_WP:
...
bool ModeTest::get_desired_location(Location& destination) const
{
    switch (_guided_mode) {
    case Guided_WP:
...
bool ModeTest::set_desired_location(const struct Location& destination,
                                      float next_leg_bearing_cd)
{
    if (g2.wp_nav.set_desired_location(destination, next_leg_bearing_cd)) {

        // handle guided specific initialisation and logging
        _guided_mode = ModeTest::Guided_WP;
        sent_notification = false;
...
void ModeTest::set_desired_heading_and_speed(float yaw_angle_cd, float target_speed)
{
    // initialisation and logging
    _guided_mode = ModeTest::Guided_HeadingAndSpeed;
...
void ModeTest::set_desired_heading_delta_and_speed(float yaw_delta_cd, float target_speed)
{
    // handle initialisation
    if (_guided_mode != ModeTest::Guided_HeadingAndSpeed) {
        _guided_mode = ModeTest::Guided_HeadingAndSpeed;
...
void ModeTest::set_desired_turn_rate_and_speed(float turn_rate_cds, float target_speed)
{
    // handle initialisation
    _guided_mode = ModeTest::Guided_TurnRateAndSpeed;
    _des_att_time_ms = AP_HAL::millis();
    _reached_destination = false;
...
void ModeTest::set_steering_and_throttle(float steering, float throttle)
{
    _guided_mode = ModeTest::Guided_SteeringAndThrottle;
...
void ModeTest::limit_set(uint32_t timeout_ms, float horiz_max)
{
...
void ModeTest::limit_init_time_and_location()
{
...
bool ModeTest::limit_breached() const
{
